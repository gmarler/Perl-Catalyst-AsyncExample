...
sub start : ChainedParent
 PathPart('echo') CaptureArgs(0) { }
  ...
  sub ws :Chained('start') Args(0)
  {
    ...
    # Now the $socket handle exists, we write the handshake response out to the
    # client.
    
    if ( $hs->is_done ) {
      say "COMPLETING WebSocket HANDSHAKE WITH CLIENT";
      $socket->write($hs->to_string);
      $socket->write($hs->build_frame(buffer => "Echo Initiated")->to_bytes);
    } else {
    ...
    $c->req->env->{'io.async.loop'}->add( $socket );
  }
